...
class PaymentSuccessView(generics.CreateAPIView):
    ...
    def create(self, request, order_oid=None):
        try:
            payload = request.data
            # Use order_oid from URL if not in payload
            order_oid = order_oid or payload.get('order_oid')
            ...
            # If no session_id provided, mark as manually verified (for testing)
            if not session_id or session_id == 'null':
                ...
                return Response({"message": "Payment manually verified"})

            # Verify Stripe payment
            try:
                session = stripe.checkout.Session.retrieve(session_id)
                print(f"Stripe session status: {session.payment_status}")
                print(f"Stripe session amount: {session.amount_total}")
                ...
                if session.payment_status == 'paid':
                    ...
                    stripe_amount = session.amount_total / 100  # Stripe amounts are in cents
                    if abs(stripe_amount - float(order.total)) > 0.01:  # Allow small difference for rounding
                        print(f"Amount mismatch: Stripe {stripe_amount}, Order {order.total}")
                        return Response(
                    ...
                    print(f"Order {order.oid} marked as paid successfully")
                    ...
                    try:
                        ...
                        subject = f"Order #{order.oid} Payment Confirmed"
                        
                        # Try to render email templates, but don't fail if they don't exist
                        try:
                            ...
                            print(f"Email template error: {template_error}")
                            # Use simple text email if templates don't exist
                            text_body = f"Your order #{order.oid} has been confirmed. Total: ${order.total}"
                            html_body = f"<h2>Order Confirmed</h2><p>Your order #{order.oid} has been confirmed.</p><p>Total: ${order.total}</p>"
                    ...
                    print(f"Stripe payment not completed. Status: {session.payment_status}")
                    return Response(
                ...
                return Response(
                    {"error": f"Stripe verification failed: {str(stripe_error)}"}, 
                    status=status.HTTP_400_BAD_REQUEST
                )
...
class StripeWebhookView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            print(f"Webhook: Checkout session completed for {session.id}")
            
            # Find the order by session ID
            try:
                order = CartOrder.objects.get(stripe_sesion_id=session.id)
                ...
                if session.payment_status == 'paid':
                    ...
                    if order.payment_status != 'paid':
                        order.payment_status = 'paid'
                        order.save()
                        print(f"Webhook: Order {order.oid} marked as paid")
                        ...
                        try:
                            ...
                            msg.attach_alternative(html_body, "text/html")
                            msg.send()
                            print(f"Webhook: Confirmation email sent to {order.email}")
                        except Exception as email_error:
                            print(f"Webhook: Email sending failed: {email_error}")
                    else:
                        print(f"Webhook: Order {order.oid} already marked as paid")
                else:
                    print(f"Webhook: Payment not completed for session {session.id}")
                    
            except CartOrder.DoesNotExist:
                print(f"Webhook: Order not found for session {session.id}")
            ...
            try:
                ...
                order.save()
                print(f"Webhook: Order {order.oid} marked as failed")
            except CartOrder.DoesNotExist:
                print(f"Webhook: Order not found for failed payment session {session.id}")
        
        return Response({"status": "success"})

@method_decorator(require_POST, name='dispatch')
class AdminLanguageSwitchView(View):
    """View to handle language switching in admin interface"""
    
    def post(self, request):
        language = request.POST.get('language', 'es')
        if language in ['es', 'pt', 'en']:
